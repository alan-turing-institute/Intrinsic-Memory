...
Every script is generated. `slurm/config.py` holds the matrix and the cluster — the starting token budget is per dataset there (`MAX_TOKENS_OVERRIDES`, with `DEFAULT_MAX_TOKENS` for the rest), `TIME_LIMIT` one value for every job. `slurm/models.py` holds what varies by model. Its three size fields — `max_model_len`, `max_num_batched_tokens`, `max_num_seqs` — are per-model defaults rather than fixed values, since what a checkpoint and a node can carry differ, and `--max_model_len` and friends override them for one generation. Edit those and rerun the generator; the scripts themselves are gitignored under `slurm/generated/`.
