...
Beyond this table, `#SBATCH --output` in every script names an absolute path under `PROJECT_DIR/results/logs`. Slurm opens that file before the job's script runs, so the directory cannot create itself: the generators make it, and print a warning instead if they cannot reach it.
