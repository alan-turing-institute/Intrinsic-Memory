...
def test_main_installs_parent_settings_from_first_expanded_experiment(
    sweep_module, monkeypatch, tmp_path
):
    args = parse(
        sweep_module, '--mas_type', 'autogen', '--task', 'fever',
        '--mas_memory', 'empty', '--seed', '1', '2', '--db_dir', str(tmp_path),
    )
    experiments = sweep_module.build_experiment_configs(args)
    seen = []

    monkeypatch.setattr(sweep_module, 'build_experiment_configs', lambda _args: experiments)
    monkeypatch.setattr(
        sweep_module,
        'install_llm_settings',
        lambda config: seen.append(config)
        or SimpleNamespace(api_base='test', max_tokens=config.max_tokens),
    )
    monkeypatch.setattr(sweep_module.results, 'check_header', lambda *_args: None)
    monkeypatch.setattr(sweep_module, 'run_experiments', lambda *_args: [])

    sweep_module.main([
        '--mas_type', 'autogen', '--task', 'fever', '--mas_memory', 'empty',
        '--seed', '1', '2', '--db_dir', str(tmp_path),
    ])

    assert seen == [experiments[0]]
